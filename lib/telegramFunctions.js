...
* @param {string} [users=""] If empty, last user with communication will receive the message
*/
function sendTelegramPosition(getState, compareTime, sendTo, lat, lon, users = "") {
    if (users == "") {
        users = getLastTelegramUser(getState, compareTime);
    }
...
* @param {string} [users=""] If empty, last user with communication will receive the message
*/
function sendTelegramVenue(getState, compareTime, sendTo, lat, lon, title, address, users = "") {
    if (users == "") {
        users = getLastTelegramUser(getState, compareTime);
    }
...
*/
function sendTelegramMessage(getState, compareTime, sendTo, msg, users = "", silent = false) {
    if (users == "") {
        users = getLastTelegramUser(getState, compareTime);
    }
...
module.exports = { TELEGRAM_USERS, sendTelegramResponse, sendTelegramPosition, sendTelegramVenue, sendTelegramMessage, getLastTelegramUser };