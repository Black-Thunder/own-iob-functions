...
let telegramQueue = [];
let isTelegramSending = false;
let telegramQueueInterval = null;
const telegramQueueIntervalTime = 1100; // 1 message every 1.1 seconds
...
function processTelegramQueue(sendTo) {
	...
	sendTo(telegramInstance, messageObj, (res) => {
		...
		if (res && res.error && res.error.code === 429) {
			const retryAfter = res.error.parameters?.retry_after || 5;

			// Wieder auf busy setzen, damit Pause eingehalten wird
			isTelegramSending = true;
			setTimeout(() => {
				isTelegramSending = false;
			}, retryAfter * 1000);
