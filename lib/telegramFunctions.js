...
function limitMessage(msg) {
	return msg.length > telegramMessageLimit
		? msg.substring(0, telegramMessageLimit - 3) + "..."
		: msg;
}
...
function sendTelegramResponse(sendTo, msg, markup, resize = true, oneTime = false, topicId = TELEGRAM_TOPIC_IDS.CONTROL) {
	sendOrQueue(sendTo, buildMessage(topicId, {
		text: limitMessage(msg),
		reply_markup: { keyboard: markup, resize_keyboard: resize, one_time_keyboard: oneTime }
	}));
}
