...
function getBrightness(existsState, getState, room) {
    ...
    if (room == commonDefines.ROOMS_EN.KUECHE) {
        ...
        brightnessLevels.push(getState(`${commonDefines.idAliasPrefix}Sensors.Luminance.${room}`).val);
        brightnessLevels.push(getState(`${commonDefines.idAliasPrefix}Sensors.Luminance.${room}PresenceSensor`).val);
