...
function isWindowOpened(existsState, getState, room) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasWindowSensor(existsState, room)) {
		return false;
	}
...
function isDoorOpened(existsState, getState, room) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasDoorSensor(existsState, room)) {
		return false;
	}
...
/**
 * @returns {number} Wert (in °C), ab dem bei längerem offenen Fenster/Tür gewarnt werden soll
 */
...
/**
 * @returns {number} Zeit (in ms), nach der bei längerem offenen Fenster/Tür gewarnt werden soll
 */
...
/**
 * @returns {number} Zeit (in ms), nach der bei längerem offenen Fenster/Tür und laufender Klimaanlage gewarnt werden soll
 */
...
function isMotionDetected(existsState, getState, room) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasMotionSensor(existsState, room)) {
		return false;
	}
...
function isMotionSensorOverridden(existsState, getState, room) {
	...
	return getState(`${commonDefines.idUserDataPrefix}LightingControl.OverrideMotionSensor${room.replace(" ", "_")}`)
		.val;
}
...
function isPresenceDetected(existsState, getState, room) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasPresenceSensor(existsState, room)) {
		return false;
	}
...
async function setPresenceSensorSensitivity(log, room, sensitivity) {
	let presenceSensorId = -1;

	switch (room) {
		case commonDefines.ROOMS.KUECHE:
			presenceSensorId = 108;
			break;
		default:
			log(`setPresenceSensorSensitivity(): Kein Präsenzmelder in ${room} bekannt.`, "error");
			return false;
	}
	...
	request.put(options, function (error, response, body) {
		if (error) {
			...
			if (body.includes("success")) {
				log(
					`setPresenceSensorSensitivity(): Präsenzmelder ${room} erfolgreich auf Sensitivität ${sensitivity} gesetzt.`,
					"info",
				);
			} else {
				log(
					`setPresenceSensorSensitivity(): Kommando an Präsenzmelder ${room} erfolgreich, aber unerwartete Antwort: ${body}`,
					"error",
				);
			}
		}
...
/**
 * @param {string} room via Rooms
 * @returns {number} Maximale Helligkeit aller vorhandenen Sensoren
 */
function getBrightness(existsState, getState, room) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasIlluminanceSensor(existsState, room)) {
		return -1;
	}
	return getState(`${commonDefines.idAliasPrefix}Sensors.Luminance.${room}.ACTUAL`).val;
}
...
/**
 * @param {string} room via Rooms
 * @param {boolean} [useRounding] flag, if value should be rounded to 1 decimal place (default = false)
 * @returns {number}
 */
function getCurrentInsideTemperature(existsState, getState, room, useRounding = false) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasTemperatureSensor(existsState, room)) {
		return false;
	}

	const sensorId = `${commonDefines.idAliasPrefix}Sensors.Temperature.${room}`;
	let sensorValue = getState(`${sensorId}.ACTUAL`).val;
	if (useRounding) {
		sensorValue = commonFunctions.roundValue(sensorValue, 1);
	}
	return sensorValue;
}
...
/**
 * @param {string} room via Rooms
 * @param {boolean} [useRounding] flag, if value should be rounded to full integer (default = false)
 * @returns {number}
 */
function getCurrentInsideHumidity(existsState, getState, room, useRounding = false) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasHumidtySensor(existsState, room)) {
		return false;
	}
...
module.exports = {
	...
	hasDoorSensor,
	isWindowOpened,
	getRoomCountWithOpenWindow,
	getRoomsWithOpenWindow,
	isAnyWindowOpened,
	areAllWindowsOpened,
	isDoorOpened,
	getRoomCountWithOpenDoor,
	getRoomsWithOpenDoor,
	isAnyDoorOpened,
	areAllDoorsOpened,
	getOpenWarningTemperatureThreshold,
	getOpenWarningTimeThreshold,
	getOpenWarningTimeAirConditionerThreshold,
	hasMotionSensor,
	isMotionDetected,
	setMotionSensorOverride,
	isMotionSensorOverridden,
	hasPresenceSensor,
	isPresenceDetected,
	setPresenceSensorSensitivity,
	/*resetPresenceSensorDetectionState,*/ hasIlluminanceSensor,
	getBrightness,
	hasTemperatureSensor,
	getCurrentInsideTemperature,
	getAllCurrentInsideTemperatures,
	hasHumidtySensor,
	getCurrentInsideHumidity,
};
