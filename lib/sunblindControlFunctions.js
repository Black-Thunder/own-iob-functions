...
let sunblindSetStateTimeout = null;
...
/**
 * @returns {boolean} Gibt an, ob sich die Markise nicht mehr in eingefahrener Position befindet
 */
function isSunblindLowered(getState) {
	const state = getSunblindState(getState);
	return (
		state == SUNBLIND_STATES.LOWERED ||
		state == SUNBLIND_STATES.LOWERING ||
		state == SUNBLIND_STATES.HALFWAY ||
		state == SUNBLIND_STATES.STOPPED
	);
}

/**
 * @returns {boolean} Gibt an, ob sich die Markise in komplett ausgefahrener Position befindet
 */
...
/**
 * @returns {boolean} Gibt an, ob sich die Markise in eingefahrener oder einfahrender Position befindet
 */
...
/**
 * @returns {boolean} Gibt an, ob sich die Markise in Bewegung befindet
 */
...
/**
 * @returns {boolean} Gibt an, ob sich die Markise physisch wirklich in eingefahrener Position befindet
 */
function isSunblindPhysicallyRetracted(getState, compareTime) {
	...
	return compareTime(new Date(primarySensorState.ts), null, ">=", new Date(backupSensorState.ts))
		? !backupSensorState.val
		: !primarySensorState.val;
}

/**
 * @returns {boolean} Gibt an, ob der Markisenatrieb geladen wird
 */
...
/**
 * @returns {boolean} Gibt zurück, ob die Markise automatisiert gesteuert werden soll
 */
 ...
 * @returns {boolean} Gibt den neuen Steuerungsmodus zurück
 */
...
/**
 * @returns {boolean} Gibt an, ob der Tageszeitplan für die Markise gilt
 */
function isDayTimeScheduleActive(getState, compareTime) {
	const currentDate = new Date();
	const dayTimeEndDate = weatherSensorFunctions.getSunsetDate(getState);
	...
	const dayTimeStartDate = new Date(currentDate.getTime());
...
/**
 * @returns {string} Gibt den softwareseitigen Status der Markise zurück (via SunblindStates)
 */
...
function setSunblindState(getState, setState, state) {
	if (getSunblindState(getState) == state) {
		return;
	}
 ...
 * @param {boolean} [shouldStateBeSet] shouldStateBeSet Flag, ob Status anschließend auf "ausgefahren" gesetzt werden soll (default=true)
 */
function lowerSunblind(getState, setState, compareTime, shouldStateBeSet = true) {
	let loweredDelay = 15 * 1000; // default 15s bis von Halbposition ausgefahren
...
/**
 * Markise stoppen
 * @param {boolean} [shouldStateBeSet] shouldStateBeSet Flag, ob Status auf "gestoppt" gesetzt werden soll (default=true)
 */
 ...
 */
function moveSunblindHalfway(log, getState, setState, compareTime) {
	if (!isSunblindPhysicallyRetracted(getState, compareTime) && !isSunblindCompletelyLowered(getState)) {
		log(
			"Markisensteuerung - Muss entweder komplett ein- oder ausgefahren sein, um auf Mittelposition zu fahren.",
			"error",
		);
		return;
	}
...
module.exports = {
	SUNBLIND_STATES,
	idSunblindCurrentState,
	idSunblindLastMovement,
	idSunblindDayTimeStart,
	idSunblindDayTimeEndOffset,
	idSunblindShouldBeControlledByScript,
	isSunblindLowered,
	isSunblindCompletelyLowered,
	isSunblindRetracted,
	isSunblindMoving,
	isSunblindPhysicallyRetracted,
	isSunblindMotorCharging,
	getSunblindState,
	setSunblindState,
	shouldSunblindBeControlledByScript,
	toggleSunblindBeControlledByScript,
	lowerSunblind,
	retractSunblind,
	stopSunblind,
	moveSunblindHalfway,
	isDayTimeScheduleActive,
};
