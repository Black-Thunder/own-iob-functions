...
async function setToDo($, getState, setState, createStateAsync, getDateObject, formatDate, content) {
	...
	const unusedEntry = sortedToDoStates.find(entry => getState(entry.id)?.val === "");
...
function clearToDoByContent($, getState, setState, content) {
	...
	const foundToDo = $(`state[state.id=${idToDoListPrefix}ToDo_*_Content]`)
		.toArray()
		.filter(id => getState(id)?.val.includes(content));
...
async function clearToDoList($, getState, setStateAsync) {
	const allToDoStates = $(`state[state.id=${idToDoListPrefix}ToDo_*_Content]`).toArray();
