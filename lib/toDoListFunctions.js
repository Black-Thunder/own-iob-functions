...
function setToDo($, getState, setState, log, getDateObject, formatDate, content) {
    const unusedToDosArray = $(`state[state.id=${idPrefix}ToDo_*_Content]`).toArray().filter((id) => getState(id)?.val === "");
...
function clearToDoByIndex(existsState, getState, setState, index) {
    const idContentState = `${idPrefix}ToDo_${index}_Content`;
...
function clearToDoByContent($, getState, setState, content) {
    if (content == "") return;
    const foundToDo = $(`state[state.id=${idPrefix}ToDo_*_Content]`).toArray().filter((id) => getState(id)?.val.includes(content));
