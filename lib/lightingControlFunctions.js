...
var isLightAliasMapInitialized = false;
...
function hasAmbientLights($, room) {
    return $(`state[state.id=alias.0.Lights.Ambience.${commonFunctions.translateRoomNameToEnglish(room)}.*.SET]`).length > 0;
}

/**
* @param {string} room via Rooms
*/
function invertAmbientLightsState($, log, getState, setState, setStateDelayed, getObject, existsState, clearStateDelayed, room, isNight) {
    const ambientOnArray = getAmbientLightStates($, log, getState, getObject, room);

    if (!ambientOnArray || ambientOnArray.length == 0) return;

    // Immer auf das erste Element gehen und diesen Zustand auswerten
    ambientOnArray[0][1] ? powerAmbientLightsOff($, log, getState, setStateDelayed, getObject, room) : powerAmbientLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight);
}
...
function invertAmbientLightsColorTemperature($, log, getState, setState, getObject, existsState, room) {
    const ambientCTs = getAmbientLightsColorTemperature($, log, getState, getObject, room);

    if (!ambientCTs || ambientCTs.length == 0) return;

    // Immer auf das erste Element gehen und diesen Zustand auswerten
    ambientCTs[0][1] == ambientLightColorTemperaturDay ? setAmbientLightsColorTemperature($, log, getState, setState, getObject, existsState, room, ambientLightColorTemperaturNight) :
        setAmbientLightsColorTemperature($, log, getState, setState, getObject, existsState, room, ambientLightColorTemperaturDay);
}
...
* @param {string} room via Rooms
*/
function setAmbientLightsDefaultBrightnessLevel($, log, getState, setState, getObject, existsState, room) {
    const ambientLightIds = getLightingIdsFromRoom($, log, getObject, room, false);
    ...
    ambientLightIds.forEach(ambientLightId => {
        setTimeout(() => {
            const defBri = getLightDefaultBrightnessLevel(getObject, existsState, ambientLightId, false);
...
/**
* @param {string} room via Rooms
* @param {number} [step=1] Step in which brightnes should be increased (default = 1) 
*/
    ...
    ambientLightIds.forEach(ambientLightId => {
        setTimeout(() => {
            if (ambientLightId.includes("alias.")) {
                ...
                if (newBrightness > 100) newBrightness = 5;
...
*/
function powerAmbientLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight) {
    // Zuerst prüfen, ob es eine passende Phoscon-Gruppe gibt, über die man schalten kann -> nur ein Befehl statt x-Einzelbefehlen
    var phosconGroupId = getPhosconGroupIdFromRoom($, room, false);

    if (phosconGroupId != "") {
        setState(phosconGroupId + idAliasPowerSuffix, true);
        ...
        setTimeout(() => {
            setAmbientLightsDefaultBrightnessLevel($, log, getState, setState, getObject, existsState, room);
    ...
    ambientLightIds.forEach(ambientLightId => {
        setTimeout(() => {
            const desiredBrightnessAmbient = getLightDefaultBrightnessLevel(getObject, existsState, ambientLightId, false);

            if (ambientLightId.includes("alias.")) {
                // Wurde sonst schon über die Gruppe geschaltet
                if (phosconGroupId == "") {
                    powerOnAliasDevice(getState, setState, existsState, ambientLightId, desiredColorTemperatureAmbient, desiredBrightnessAmbient);
                }
            }
            else {
                powerOnYeelight(getState, setState, clearStateDelayed, ambientLightId, isNight, "color", desiredColor, desiredBrightnessAmbient, null, true);
                powerOnYeelight(getState, setState, clearStateDelayed, ambientLightId, isNight, "ct", desiredColorTemperatureAmbient, desiredBrightnessAmbient, null, true);
            }
        }, 0);
    });
}
...
/**
* @param {string} room via Rooms
* @param {number} [delay=0] Delay when powering off (default = 0) 
*/
function powerAmbientLightsOff($, log, getState, setStateDelayed, getObject, room, delay = 0) {
    // Zuerst prüfen, ob es eine passende Phoscon-Gruppe gibt, über die man schalten kann -> nur ein Befehl statt x-Einzelbefehlen
    var phosconGroupId = getPhosconGroupIdFromRoom($, room, false);

    if (phosconGroupId != "") {
        setState(phosconGroupId + idAliasPowerSuffix, false);
    }
...
function setAmbientLightsDefaultColorTemperature($, log, getState, setState, getObject, existsState, room, isNight) {
    ...
    ambientLightIds.forEach(ambientLightId => {
        setTimeout(() => {
            var defValue = getDesiredColorTemperature(room, false, isNight); // zunächst die fix hinterlegte Farbtemperatur als Standard nehmen
                ...
                if (isYeelightOn(getState, ambientLightId)) {
                    ...
                    if (existsState(ctYeelightStateId)) {
                        if (lightAliasMap.has(ambientLightId)) {
                            ...
                            const defaultValueEnum = commonFunctions.getDefaultValueEnum(existsState, getObject, ctAliasStateId);
                            defValue = defaultValueEnum != "" ? defaultValueEnum : defValue;
                        }
                    }
...
/**
* @param {string} room via Rooms
* @return {boolean[]} Array with all ambient light IDs und flag indicating if they're powered on and reachable [ID, isPoweredOn, isReachable] (sort order: alias -> yeelight)
*/
function getAmbientLightStates($, log, getState, getObject, room) {
    ...
    ambientLightIds.forEach(ambientLightId => {
        if (ambientLightId.includes("alias.")) {
            var element = [ambientLightId, isAliasDeviceOn(getState, ambientLightId), isAliasDeviceReachable(getState, ambientLightId)];
            areOn.push(element);
        }
        else {
            var element = [ambientLightId, isYeelightOn(getState, ambientLightId), isYeelightReachable(getState, ambientLightId)];
            areOn.push(element);
        }
    });
...
/**
* @param {string} room via Rooms
* @return {boolean} flag indicating if any ambient light is powered on and reachable
*/
function isAnyAmbientLightPoweredOn($, log, getState, getObject, room) {
    ...
    ambientLightStates.forEach(state => {
        if (state[2] && state[1]) isAnyOn = true;
...
/**
* @param {string} room via Rooms
* @return {number[]} Array with all ambient light IDs und color temperatures [ID, colorTemperature] (sort order: alias -> yeelight)
*/
function getAmbientLightsColorTemperature($, log, getState, getObject, room) {
    const ambientLightIds = getLightingIdsFromRoom($, log, getObject, room, false);
    ...
    ambientLightIds.forEach(ambientLightId => {
        if (ambientLightId.includes("alias.")) {
            var element = [ambientLightId, getAliasDeviceColorTemperature(getState, ambientLightId)];
            areOn.push(element);
        }
        else {
            var element = [ambientLightId, getYeelightColorTemperature(getState, ambientLightId)];
...
function hasMainLights($, room) {
    return $(`state[state.id=alias.0.Lights.Main.${commonFunctions.translateRoomNameToEnglish(room)}.*.SET]`).length > 0;
}

/**f
* @param {string} room via Rooms
*/
function invertMainLightsState($, log, getState, setState, setStateDelayed, getObject, existsState, clearStateDelayed, room, isNight) {
    const mainOnArray = getMainLightStates($, log, getState, getObject, room);

    if (!mainOnArray || mainOnArray.length == 0) return false;

    // Immer auf das erste Element gehen und diesen Zustand auswerten
    mainOnArray[0][1] ? powerMainLightsOff($, log, getState, setStateDelayed, getObject, room) : powerMainLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight);
}
...
function invertMainLightsColorTemperature($, log, getState, setState, getObject, existsState, room) {
    const mainCTs = getMainLightsColorTemperature($, log, getState, getObject, room);

    if (!mainCTs || mainCTs.length == 0) return false;

    mainCTs[0][1] == mainLightColorTemperaturDay ? setMainLightsColorTemperature($, log, getState, setState, getObject, existsState, room, mainLightColorTemperaturNight) :
        setMainLightsColorTemperature($, log, getState, setState, getObject, existsState, room, mainLightColorTemperaturDay);
}
...
* @param {string} room via Rooms
*/
function setMainLightsDefaultBrightnessLevel($, log, getState, setState, getObject, existsState, room) {
    const mainLightIds = getLightingIdsFromRoom($, log, getObject, room, true);
    ...
    mainLightIds.forEach(mainLightId => {
        setTimeout(() => {
            const defBri = getLightDefaultBrightnessLevel(getObject, existsState, mainLightId, true);
...
*/
function powerMainLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight) {
    // Zuerst prüfen, ob es eine passende Phoscon-Gruppe gibt, über die man schalten kann -> nur ein Befehl statt x-Einzelbefehlen
    var phosconGroupId = getPhosconGroupIdFromRoom($, room, true);
    ...
    if (phosconGroupId != "") {
        ...
        // Kurz verzögern, da Setzen der Lichtfarbe nur im eingeschalteten Zustand erlaubt ist und es sonst evtl. zu einem Timingproblem kommt
        setTimeout(() => {
            setMainLightsDefaultBrightnessLevel($, log, getState, setState, getObject, existsState, room);
    ...
    mainLightIds.forEach(mainLightId => {
        setTimeout(() => {
            const desiredBrightness = getLightDefaultBrightnessLevel(getObject, existsState, mainLightId, true);

            if (mainLightId.includes("alias.")) {
                // Wurde sonst schon über die Gruppe geschaltet
                if (phosconGroupId == "") {
                    powerOnAliasDevice(getState, setState, existsState, mainLightId, desiredColorTemperature, desiredBrightness);
                }
            }
            else {
                powerOnYeelight(getState, setState, clearStateDelayed, mainLightId, isNight, "ct", desiredColorTemperature, desiredBrightness);
            }
        }, 0);
    });
}
...
* @param {number} [delay=0] Delay when powering off (default = 0) 
*/
function powerMainLightsOff($, log, getState, setStateDelayed, getObject, room, delay = 0) {
    // Zuerst prüfen, ob es eine passende Phoscon-Gruppe gibt, über die man schalten kann -> nur ein Befehl statt x-Einzelbefehlen
    var phosconGroupId = getPhosconGroupIdFromRoom($, room, true);
...
function setMainLightsDefaultColorTemperature($, log, getState, setState, getObject, existsState, room, isNight) {
    ...
    mainLightIds.forEach(mainLightId => {
        setTimeout(() => {
            var defValue = getDesiredColorTemperature(room, true, isNight); // zunächst die fix hinterlegte Farbtemperatur als Standard nehmen
...
/**
* @param {string} room via Rooms
* @return {boolean[]} Array with all main light IDs und flag indicating if they're powered on and reachable [ID, isPoweredOn, isReachable] (sort order: alias -> yeelight)
*/
function getMainLightStates($, log, getState, getObject, room) {
    ...
    mainLightIds.forEach(mainLightId => {
        if (mainLightId.includes("alias.")) {
            var element = [mainLightId, isAliasDeviceOn(getState, mainLightId), isAliasDeviceReachable(getState, mainLightId)];
            areOn.push(element);
        }
        else {
            var element = [mainLightId, isYeelightOn(getState, mainLightId), isYeelightReachable(getState, mainLightId)];
            areOn.push(element);
        }
    });
...
/**
* @param {string} room via Rooms
* @return {boolean} flag indicating if any main light is powered on and reachable
*/
...
/**
* @param {string} room via Rooms
* @return {number[]}  Array with all main light IDs und color temperatures [ID, colorTemperature] (sort order: alias -> yeelight)
*/
function getMainLightsColorTemperature($, log, getState, getObject, room) {
    const mainLightIds = getLightingIdsFromRoom($, log, getObject, room, true);
    var colorTemperatures = [];
    ...
    mainLightIds.forEach(mainLightId => {
        if (mainLightId.includes("alias.")) {
            var element = [mainLightId, getAliasDeviceColorTemperature(getState, mainLightId)];
            colorTemperatures.push(element);
        }
        else {
            var element = [mainLightId, getYeelightColorTemperature(getState, mainLightId)];
            colorTemperatures.push(element);
        }
    });
...
* @param {number} [param1=0] default = 0
* @param {number} [param2=0] default = 0
* @param {number} [param3=0] default = 0
* @param {boolean} [isAmbient=false] Flag, if is an ambient light or not (default = false)
*/
function powerOnYeelight(getState, setState, clearStateDelayed, id, isNight, className = "", param1 = 0, param2 = 0, param3 = 0, isAmbient = false) {
    ...
    // Mittels set_scene einschalten
        ...
        switch (className) {
            case "color": sceneJSON = "[\"" + className + "\", " + param1 + ", " + param2 + "]"; break;
            case "hsv": sceneJSON = "[\"" + className + "\", " + param1 + ", " + param2 + + ", " + param3 + "]"; break;
            case "ct": sceneJSON = "[\"" + className + "\", " + param1 + ", " + param2 + "]"; break;
            case "cf": sceneJSON = "[\"" + className + "\", " + param1 + ", " + param2 + + ", " + param3 + "]"; break;
            case "auto_delay_off": sceneJSON = "[\"" + className + "\", " + param1 + ", " + param2 + "]"; break;
            default: break;
        }
...
* @return {number}
*/
function getYeelightColorTemperature(getState, id) {
    const isPendant = isYeelightPendant(id);
    const ctSuffix = isPendant ? idYeelightBgColorTemperatureSuffix : idYeelightColorTemperatureSuffix;
    return getState(id + ctSuffix).val;
}
...
/**
* @param {string} id
* @param {number} [ct=-1] Change color temperature directly if desired (default = -1/don't change)
* @param {number} [brightness=-1] Change brightness directly if desired (default = -1/don't change)
*/
...
/**
* @param {string} id
* @param {boolean} [asKelvin=true] default = true
* @return {number} Color temperature (as Kelvin if asKelvin is set to true, otherwise as mired)
*/
function getAliasDeviceColorTemperature(getState, id, asKelvin = true) {
...
* @param {string} id
* @param {number} ct
* @param {boolean} [connvertToMired=false] Flag, if value should be converted from Kelvin to mired first (default = false)
*/
function setColorTemperatureAliasDevice(getState, setState, existsState, id, ct, connvertToMired = false) {
    if (!isAliasDeviceReachable(getState, id) || !existsState(id + idAliasColorTemperatureSuffix) || !isAliasDeviceOn(getState, id)) { // Wenn ausgeschaltet, würde eine Warnung im Log erscheinen, da nur im eingeschalteten Zustand das Setzen erlaubt ist
        return;
    }
...
function fillLightAliasMap($, log, getObject) {
    if (isLightAliasMapInitialized) return;

    Object.values(commonDefines.ROOMS_WITH_LIGHTING).forEach(room => {
        const mainLightAliasIds = getLightingIdsFromRoom($, log, getObject, room, true, false, false);

        mainLightAliasIds.forEach(mainLightAliasId => {
            const lightObj = getObject(`${mainLightAliasId}.SET`);
            var realYeelightId = lightObj.common && lightObj.common.alias ? lightObj.common.alias.id : "";

            if (realYeelightId.includes("yeelight-2.0")) {
                realYeelightId = realYeelightId.replace(".control.power", "");
                realYeelightId = realYeelightId.replace(".control.bg_power", "");

                // Nötig, da ansonsten jede Pendelleuchte fälschlicherweise nur einmal aufgenommen werden würde
        ...
        const ambientLightAliasIds = getLightingIdsFromRoom($, log, getObject, room, false, false, false);
        ambientLightAliasIds.forEach(ambientLightAliasId => {
            const lightObj = getObject(`${ambientLightAliasId}.SET`);
            var realYeelightId = lightObj.common && lightObj.common.alias ? lightObj.common.alias.id : "";
...
*/
function getLightDefaultBrightnessLevel(getObject, existsState, lightId, isMainLight) {
    const isAliasDevice = lightId.includes("alias.");
    const isPendant = !isAliasDevice && isYeelightPendant(lightId);
    const mappedId = lightAliasMap.has(isPendant ? (isMainLight ? `${lightId}${lightAliasMapPendantMainSuffix}` : `${lightId}${lightAliasMapPendantAmbienceSuffix}`) : lightId) ?
        lightAliasMap.get(isPendant ? (isMainLight ? `${lightId}${lightAliasMapPendantMainSuffix}` : `${lightId}${lightAliasMapPendantAmbienceSuffix}`) : lightId) : "NOT_MAPPED";
    const room = commonFunctions.getAssignedRoomFromState(getObject, mappedId);

    var defValue = getDesiredBrightness(room, isMainLight); // zunächst die fix hinterlegte Helligkeit als Standard nehmen
...
function getDesiredBrightness(room, isMainLight) {
    switch (room) {
        case commonDefines.ROOMS.KUECHE: return isMainLight ? 1 : 100;
        case commonDefines.ROOMS.GANG: return 60;
        default: return isMainLight ? 50 : 100;
    }
}
...
function getBrightnessThreshold(room, isMainLight) {
    switch (room) {
        case commonDefines.ROOMS.KUECHE: return isMainLight ? 10 : 45;
        case commonDefines.ROOMS.GANG: return 25;
        default: return isMainLight ? 10 : 100;
    }
}
...
* @return {string[]} Array mit IDs aller Beleuchtungen (Sortierung: alias -> yeelight)
*/
function getLightingIdsFromRoom($, log, getObject, room, searchMainLights, logging = true, separateYeelights = true) {
    var lightIds = "";
    var aliasIds = [];
    var yeelightIds = [];

    // Aus dem Cache lesen, wenn bestückt
    if (isLightAliasMapInitialized) {
        lightAliasMap.forEach((value, key, map) => {
            if (value.includes(`${searchMainLights ? ".Main" : ".Ambience"}.${commonFunctions.translateRoomNameToEnglish(room)}.`)) {
                if (key.includes("yeelight-2.0.")) {
                    key = key.replace(`${searchMainLights ? lightAliasMapPendantMainSuffix : lightAliasMapPendantAmbienceSuffix}`, "");
                    yeelightIds.push(separateYeelights ? key : value); // Yeelights gesondert behandeln und spezifische ID statt Alias-ID ausgeben
                }
                else {
                    aliasIds.push(separateYeelights ? key : value); // Yeelights gesondert behandeln und spezifische ID statt Alias-ID ausgeben
                }
            }
        });
    }
    else {
        $(`state[state.id=alias.0.Lights.${searchMainLights ? 'Main' : 'Ambience'}.${commonFunctions.translateRoomNameToEnglish(room)}.*.SET]`).each(function (id) {
            // Yeelights gesondert behandeln und spezifische ID statt Alias-ID ausgeben
            if (separateYeelights) {
                const obj = getObject(id);
                const aliasId = obj.common.alias.id;

                // Sonderfall: Yeelights werden direkt über die jeweilige ID im yeelight-Adapter gesteuert
                if (aliasId.includes("yeelight-2.0.")) {
                    id = aliasId.replace(".control.power", "");
                    id = id.replace(".control.bg_power", ""); // Bei Pendelleuchten das Ambientelicht
                    yeelightIds.push(id);
                }
                // Alle anderen werden über alias gesteuert
                else {
                    id = id.replace(".SET", "");
                    aliasIds.push(id);
                }
            }
            // Einfach direkt als Liste von Alias-IDs ausgeben
            else {
                id = id.replace(".SET", "");
                aliasIds.push(id);
            }
    ...
    if (lightIds.length == 0 && logging) {
        log(`getLightingIdsFromRoom(${room}, ${searchMainLights}, ${logging}, ${separateYeelights}): Keine ${searchMainLights ? 'Haupt' : 'Ambiente'}beleuchtung in Raum '${String(room)}'`, "error");
    }
...
/**
* Bestimmt dynamisch die ID der Phoscon-Leuchtengruppe im gewünschten Raum (erfasst alle unter alias.0.Lights.Groups.Main/Ambience.* gemappten Gruppen)
* @param {string} room via Rooms
* @param {boolean} searchMainLights Flag, if main light ids should be returned
* @return {string} ID der Phoscon-Leuchtengruppe
*/
function getPhosconGroupIdFromRoom($, room, searchMainLights) {
    var phosconGroupId = "";

    $(`state[state.id=alias.0.Lights.Groups.${searchMainLights ? "Main" : "Ambience"}.${commonFunctions.translateRoomNameToEnglish(room)}.*.SET]`).each(function (id) {
        phosconGroupId = id.replace(".SET", "");
        return false; // frühzeitig rausspringen, da gefunden
    });

    return phosconGroupId;
}
...
function setWasPoweredOnByMotion(setState, room, isMain, val) {
    if (!commonDefines.ROOMS_WITH_MOTION_SENSOR.includes(room)) return;
    setState(`${commonDefines.idUserDataPrefix}LightingControl.WasPoweredOnByMotion${room}${(isMain ? "Main" : "Ambient")}`, val);
}
...
function getWasPoweredOnByMotion(getState, room, isMain) {
    if (!commonDefines.ROOMS_WITH_MOTION_SENSOR.includes(room)) return;
    return getState(`${commonDefines.idUserDataPrefix}LightingControl.WasPoweredOnByMotion${room}${(isMain ? "Main" : "Ambient")}`).val;
}

//#endregion

module.exports = {
    YEELIGHT_SCENES, hasAmbientLights, invertAmbientLightsState, invertAmbientLightsColorTemperature, setAmbientLightsBrightnessLevel, setAmbientLightsDefaultBrightnessLevel, increaseAmbientLightsBrightnessLevel,
    powerAmbientLightsOn, powerAmbientLightsOff, setAmbientLightsColorTemperature, setAmbientLightsDefaultColorTemperature, getAmbientLightStates, isAnyAmbientLightPoweredOn, getAmbientLightsColorTemperature,
    hasMainLights, invertMainLightsState, invertMainLightsColorTemperature, setMainLightsBrightnessLevel, setMainLightsDefaultBrightnessLevel, increaseMainLightsBrightnessLevel, powerMainLightsOn,
    powerMainLightsOff, setMainLightsColorTemperature, setMainLightsDefaultColorTemperature, setMainLightsMoonMode, getMainLightStates, isAnyMainLightPoweredOn, getMainLightsColorTemperature,
    isYeelightPendant, powerOnYeelight, powerOffYeelight, setColorTemperatureYeelight, triggerYeelightScene, isYeelightOn, isYeelightReachable, getYeelightColorTemperature, getYeelightBrightness, setYeelightBrightness,
    setYeelightMoonMode, powerOnAliasDevice, powerOffAliasDevice, isAliasDeviceOn, isAliasDeviceReachable, getAliasDeviceColorTemperature, setColorTemperatureAliasDevice, getAliasDeviceBrightness, setAliasDeviceBrightness,
    getDesiredColor, getDesiredColorAsHex, getDesiredColorTemperature, getLightDefaultBrightnessLevel, getDesiredBrightness, getBrightnessThreshold, convertKelvinToMired, convertMiredToKelvin, getLightingIdsFromRoom, getPhosconGroupIdFromRoom,
    setWasPoweredOnByMotion, getWasPoweredOnByMotion, fillLightAliasMap
};