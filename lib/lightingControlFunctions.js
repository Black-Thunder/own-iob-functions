 ...
 */
function fillLightAliasMap($, getObject) {
	...
	Object.values(commonDefines.ROOMS_WITH_LIGHTING).forEach(room => {
		...
		["Main", "Ambience"].forEach(type => {
			const typeLightIds = [];
			const stateArray = $(`state[state.id=alias.0.Lights.${type}.${roomEng}.*${idAliasPowerSuffix}]`);

			stateArray.forEach(id => {
				const obj = getObject(id);
				if (!obj || !obj.common) {
					return;
				}

				let aliasId = obj.common.alias ? obj.common.alias.id : id.replace(idAliasPowerSuffix, "");

				// Yeelight-Sonderfall
				if (aliasId.includes("yeelight-2.0.")) {
					aliasId = aliasId.replace(".control.power", "").replace(".control.bg_power", "");
					if (isYeelightPendant(aliasId)) {
						aliasId +=
							type === "Main" ? lightAliasMapPendantMainSuffix : lightAliasMapPendantAmbienceSuffix;
					}
				}

				typeLightIds.push({ aliasId, originalId: id.replace(idAliasPowerSuffix, "") });
			});

			typeLightIds.forEach(entry => {
				if (!lightAliasMap.has(entry.aliasId)) {
					lightAliasMap.set(entry.aliasId, entry.originalId);
				}
			});
		});
	});
