...
function fillLightAliasMap($, getObject) {
	...
	Object.values(commonDefines.ROOMS_WITH_LIGHTING).forEach(room => {
		...
		["Main", "Ambience"].forEach(type => {
			const lightIds = $(`state[state.id=alias.0.Lights.${type}.${roomEng}.*${idAliasPowerSuffix}]`)
				.map(id => {
					...
					return { aliasId, originalId: id.replace(idAliasPowerSuffix, "") };
				})
				.filter(Boolean);

			lightIds.forEach(entry => {
