...
function setAmbientLightsDefaultBrightnessLevel($, log, getEnums, getState, setState, getObject, existsState, room) {
	...
	ambientLightIds.forEach(ambientLightId => {
		setTimeout(() => {
			const defBri = getLightDefaultBrightnessLevel(getEnums, getObject, existsState, ambientLightId, false);
...
function setMainLightsDefaultBrightnessLevel($, log, getEnums, getState, setState, getObject, existsState, room) {
	...
	mainLightIds.forEach(mainLightId => {
		setTimeout(() => {
			const defBri = getLightDefaultBrightnessLevel(getEnums, getObject, existsState, mainLightId, true);
...
function powerMainLightsOn(
	...
	mainLightIds.forEach(mainLightId => {
		setTimeout(() => {
			const desiredBrightness = getLightDefaultBrightnessLevel(
				getEnums,
				getObject,
				existsState,
				mainLightId,
...
function fillLightAliasMap($, log, getObject) {
	...
	const processLightIds = (lightAliasIds, isMain) => {
		lightAliasIds.forEach(aliasId => {
			const obj = getObject(`${aliasId}${idAliasPowerSuffix}`);
			if (!obj || !obj.common) {
				return;
			}

			let realYeelightId = obj.common.alias?.id || "";

			if (realYeelightId.includes("yeelight-2.0")) {
				realYeelightId = realYeelightId.replace(/\.control\.power|\.control\.bg_power/g, "");
				...
				if (isYeelightPendant(realYeelightId)) {
					realYeelightId += isMain ? lightAliasMapPendantMainSuffix : lightAliasMapPendantAmbienceSuffix;
				}

				if (!lightAliasMap.has(realYeelightId)) {
					lightAliasMap.set(realYeelightId, aliasId);
				}
			} else {
				if (!lightAliasMap.has(aliasId)) {
					lightAliasMap.set(aliasId, aliasId);
				}
			}
		});
	};
	...
	Object.values(commonDefines.ROOMS_WITH_LIGHTING).forEach(room => {
		const mainIds = getLightingIdsFromRoom($, log, getObject, room, true, false, false);
		const ambientIds = getLightingIdsFromRoom($, log, getObject, room, false, false, false);

		processLightIds(mainIds, true);
		processLightIds(ambientIds, false);
	});
 ...
 */
function getLightDefaultBrightnessLevel(getEnums, getObject, existsState, lightId, isMainLight) {
	...
	const mappedId = lightAliasMap.has(
		...
		: "NOT_MAPPED";
	const room = commonFunctions.getRoomFromState(getEnums, mappedId);
...
function getLightingIdsFromRoom($, log, getObject, room, searchMainLights, logging = true, separateYeelights = true) {
	// Wenn das Mapping noch nicht initialisiert ist, einmalig füllen
	if (!isLightAliasMapInitialized) {
	...
	const aliasIds = [];
	const yeelightIds = [];
	const roomEng = commonFunctions.translateRoomNameToEnglish(room);
	const suffixToCheck = searchMainLights ? ".Main" : ".Ambience";

	// Durch das Mapping iterieren
	lightAliasMap.forEach((value, key) => {
		// Prüfen, ob der Alias zum Raum und Lichttyp passt
		if (value.includes(`${suffixToCheck}.${roomEng}.`)) {
			if (key.includes("yeelight-2.0.")) {
				// Pendelleuchten-Suffix entfernen, falls vorhanden
				const cleanedKey = key.replace(
					searchMainLights ? lightAliasMapPendantMainSuffix : lightAliasMapPendantAmbienceSuffix,
					"",
				);
				yeelightIds.push(separateYeelights ? cleanedKey : value);
			} else {
	...
	const lightIds = aliasIds.concat(yeelightIds);
