...
function invertAmbientLightsState(
	...
	const isOn = ambientOnArray.find(ambientInfo => ambientInfo[2])?.[1] ?? false;

	// Use object destructuring for cleaner function calls
	const commonParams = {
		$,
		log,
		getState,
...
function increaseAmbientLightsBrightnessLevel($, log, getState, setState, getObject, existsState, room, step = 1) {
	...
	ambientLightIds.forEach(ambientLightId => {
		setTimeout(() => {
			if (ambientLightId.includes("alias.")) {
				let newBrightness = getAliasDeviceBrightness(getState, room) + step;
				if (newBrightness > 100) {
					newBrightness = 5;
				}
				setAliasDeviceBrightness(getState, setState, existsState, ambientLightId, newBrightness);
			} else {
				let newBrightness = getYeelightBrightness(getState, room) + step;
				if (newBrightness > 100) {
					newBrightness = 5;
				}
		...
		setTimeout(() => {
			...
			if (ambientLightId.includes("alias.")) {
				if (isAliasDeviceOn(getState, ambientLightId)) {
					...
					defValue = defaultValueEnum != null ? defaultValueEnum : defValue;
...
function getAmbientLightsColorTemperature($, log, getState, getObject, existsState, room) {
	...
	ambientLightIds.forEach(ambientLightId => {
		if (ambientLightId.includes("alias.")) {
			const element = [ambientLightId, getAliasDeviceColorTemperature(getState, existsState, ambientLightId)];
			colorTemperatures.push(element);
		} else {
			const element = [ambientLightId, getYeelightColorTemperature(getState, existsState, ambientLightId)];
			colorTemperatures.push(element);
		}
	});
...
function increaseMainLightsBrightnessLevel($, log, getState, setState, getObject, existsState, room, step = 1) {
	...
	mainLightIds.forEach(mainLightId => {
		setTimeout(() => {
			if (mainLightId.includes("alias.")) {
				let newBrightness = getAliasDeviceBrightness(getState, mainLightId) + step;
				if (newBrightness > 100) {
					newBrightness = 5;
				}
				setAliasDeviceBrightness(getState, setState, existsState, mainLightId, newBrightness);
			} else {
				let newBrightness = getYeelightBrightness(getState, mainLightId) + step;
				if (newBrightness > 100) {
					newBrightness = 5;
				}
...
function isAnyMainLightPoweredOn($, log, getState, getObject, room) {
	...
	mainLightStates.some(function (mainState) {
...
function getMainLightsColorTemperature($, log, getState, getObject, existsState, room) {
	...
	mainLightIds.forEach(mainLightId => {
		if (mainLightId.includes("alias.")) {
			const element = [mainLightId, getAliasDeviceColorTemperature(getState, existsState, mainLightId)];
			colorTemperatures.push(element);
		} else {
			const element = [mainLightId, getYeelightColorTemperature(getState, existsState, mainLightId)];
			colorTemperatures.push(element);
		}
	});
