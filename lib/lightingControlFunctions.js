...
function powerAmbientLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight) {
    ...
    var lightingGroupId = getLightingGroupIdFromRoom($, room, false);
...
function powerMainLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight) {
    // Zuerst prüfen, ob es eine passende Lichtergruppe gibt, über die man schalten kann -> nur ein Befehl statt x-Einzelbefehlen
    var lightingGroupId = getLightingGroupIdFromRoom($, room, true);
...
* @return {number} Color temperature (returned as Kelvin if "asKelvin" is set to true, otherwise as mired); -1 if not supported
*/
...
* @param {string} room via Rooms
* @param {boolean} searchMainLights Flag, if main light ids should be returned
* @return {string} ID der Lichtergruppe
*/
function getLightingGroupIdFromRoom($, room, searchMainLights) {
    ...
    $(`state[state.id=alias.0.Lights.Groups.${searchMainLights ? "Main" : "Ambience"}.${commonFunctions.translateRoomNameToEnglish(room)}.*.SET]`).each(function (id) {
        lightingGroupId = id.replace(".SET", "");
        return false; // frühzeitig rausspringen, da gefunden
    });
...
module.exports = {
    ...
    getDesiredColor, getDesiredColorAsHex, getDesiredColorTemperature, getLightDefaultBrightnessLevel, getDesiredBrightness, getBrightnessThreshold, convertKelvinToMired, convertMiredToKelvin, getLightingIdsFromRoom, getLightingGroupIdFromRoom,
