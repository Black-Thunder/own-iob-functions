...
function hasAmbientLights($, room) {
    return $(`state[state.id=alias.0.Lights.Ambience.${commonFunctions.translateRoomNameToEnglish(room)}.*${idAliasPowerSuffix}]`).length > 0;
}
...
function invertAmbientLightsState($, log, getState, setState, setStateDelayed, getObject, existsState, clearStateDelayed, room, isNight) {
    ...
    isOn ? powerAmbientLightsOff($, log, getState, setState, setStateDelayed, getObject, room) : powerAmbientLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight);
}
...
function getAmbientLightStates($, log, getState, getObject, room) {
    ...
    if (lightingGroupId != "") {
        var element = [lightingGroupId, getState(`${lightingGroupId}${idAliasPowerSuffix}`).val, true]; // Gruppe immer als erreichbar markieren
        areOn.push(element);
    }
...
function hasMainLights($, room) {
    return $(`state[state.id=alias.0.Lights.Main.${commonFunctions.translateRoomNameToEnglish(room)}.*${idAliasPowerSuffix}]`).length > 0;
}
...
function invertMainLightsState($, log, getState, setState, setStateDelayed, getObject, existsState, clearStateDelayed, room, isNight) {
    ...
    // Das erste Element finden, das erreichbar ist (Index 2) und diesen Zustand auswerten (Index 1)
    var isOn = false;
    mainOnArray.some(function (mainInfo, index) {
        if (mainInfo[2]) {
            isOn = mainInfo[1];
            return true;
        }
    ...
    isOn ? powerMainLightsOff($, log, getState, setState, setStateDelayed, getObject, room) : powerMainLightsOn($, log, getState, setState, getObject, existsState, clearStateDelayed, room, isNight);
}
...
function getLightingIdsFromRoom($, log, getObject, room, searchMainLights, logging = true, separateYeelights = true) {
    ...
    if (isLightAliasMapInitialized) {
        ...
        $(`state[state.id=alias.0.Lights.${searchMainLights ? 'Main' : 'Ambience'}.${commonFunctions.translateRoomNameToEnglish(room)}.*${idAliasPowerSuffix}]`).each(function (id) {
...
function getLightingGroupIdFromRoom($, room, searchMainLights) {
    ...
    $(`state[state.id=alias.0.Lights.Groups.${searchMainLights ? "Main" : "Ambience"}.${commonFunctions.translateRoomNameToEnglish(room)}${idAliasPowerSuffix}]`).each(function (id) {
