...
/**
 * @param {boolean} [useRailing] Flag, if railing sensor should be used or wall sensor (default = true)
 * @param {boolean} [returnRaw] Flag, if raw values should be returned or rounded values (to 1 decimal place) (default = false)
 * @returns {number} Current outside temperature or -1 if all sensors are not reachable
 */
...
/**
 * @param {boolean} [useRailing] Flag, if railing sensor should be used or wall sensor (default = true)
 * @param {boolean} [returnRaw] Flag, if raw values should be returned or rounded values (no decimal places) (default = false)
 * @returns {number}
 */
function getCurrentOutsideHumidity(getState, useRailing = true, returnRaw = false) {
	const rawHumidity = useRailing
		? getState("alias.0.Sensors.Humidity.OutdoorRailing.ACTUAL").val
		: getState("alias.0.Sensors.Humidity.OutdoorWall.ACTUAL").val;
 ...
 * @returns {number} Wertebereich 0-100.000
 */
function getCurrentOutsideIllumination(getState) {
	const luxValuesArray = [];

	// Sensor Balkonbrüstung (unreachable wird über den Temperatursensor abgefragt)
	// Eigentlicher Wertebereich: 0-16367 -> Max wurde aber noch nie erreicht, daher nur bis 10000 annehmen
	if (!getState("alias.0.Sensors.Temperature.OutdoorRailing.UNREACH").val) {
		...
		const minRawValue = 0;
		const maxRawValue = rawValue <= 10000 ? 10000 : rawValue;
		const mappedValue = commonFunctions.mapRange(rawValue, minRawValue, maxRawValue, 0, 100000);
...
/**
 * @returns {string} Windrichtung als Kürzel (z.B. "NNW")
 */
...
/**
 * @returns {number} Atmosphärischer Luftdruck auf Höhe des Sensors in hPa
 */
...
/**
 * @returns {number} Luftdruck auf Meeereshöhe (basierend auf dem atmosphärischen Druck) in hPa
 */
...
function getSunsetDate(getState) {
	...
	return splittedSunset && splittedSunset.length == 2
		? new Date(today.getFullYear(), today.getMonth(), today.getDate(), splittedSunset[0], splittedSunset[1])
		: null;
}

module.exports = {
	getCurrentOutsideTemperature,
	getCurrentOutsideHumidity,
	getCurrentOutsideIllumination,
	getCurrentOutsideWindSpeed,
	getCurrentOutsideWindDirection,
	getCurrentAtmosphericBaromtericPressure,
	getCurrentMSLBaromtericPressure,
	isRaining,
	getSunsetDate,
};
