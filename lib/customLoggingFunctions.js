...
function backupAndClearCurrentLog(log, getState, setState, getDateObject, formatDate, writeFile, sendTo) {
    saveCurrentLogToFile(log, getState, setState, getDateObject, formatDate, writeFile, sendTo, wasSuccessful => { if (wasSuccessful) clearCurrentLog(setState); });
}
...
function saveCurrentLogToFile(log, getState, setState, getDateObject, formatDate, writeFile, sendTo, cb) {
    ...
    writeFile(userdataRootDirectory, `${subfolder}/${currentDateObj.getFullYear()}/${currentDateObj.getMonth() + 1}/${fileNamePrefix}${currentDateString}.txt`, currentLogList, function (error) {
