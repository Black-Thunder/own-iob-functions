...
function addCustomLogEntry(getState, setState, getDateObject, formatDate, logLevel, logText) {
	...
	let logList = getState(idLoggingPrefix + idCurrentEventList).val;
	...
	const newEntry = `<font size="4">${entryDateTime}: <font color="${logColor}">${logLevel}</font> - ${logText}</font>`;
	logList = `${newEntry}<br>${logList}`;

	setState(idLoggingPrefix + idCurrentEventList, logList, () => {
		isBusyWriting = false;
	});
}

function backupAndClearCurrentLog(log, getState, setState, getDateObject, formatDate, writeFile) {
	saveCurrentLogToFile(log, getState, setState, getDateObject, formatDate, writeFile, wasSuccessful => {
		if (wasSuccessful) {
			clearCurrentLog(setState);
		}
	});
}
...
function saveCurrentLogToFile(log, getState, setState, getDateObject, formatDate, writeFile, cb) {
	const userdataRootDirectory = commonDefines.idUserDataPrefix.substring(
		0,
		commonDefines.idUserDataPrefix.length - 1,
	); // ohne letzten "."
	let currentLogList = getState(idLoggingPrefix + idCurrentEventList).val;
	...
	writeFile(
		userdataRootDirectory,
		`${subfolder}/${currentDateObj.getFullYear()}/${currentDateObj.getMonth() + 1 < 10 ? `0${currentDateObj.getMonth() + 1}` : `${currentDateObj.getMonth() + 1}`}/${fileNamePrefix}${currentDateString}.txt`,
		currentLogList,
		function (error) {
