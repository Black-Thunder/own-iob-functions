...
const minDefaultACTargetCoolTempPresent = 23.5; // Standardwert der minimalen Soll-Temperatur (Kühlen), die bei Anwesenheit automatisch eingestellt werden soll
const maxDefaultACTargetHeatTempPresent = 22.0; // Standardwert der maximalen Soll-Temperatur (Heizen), die bei Anwesenheit automatisch eingestellt werden soll
...
function adjustACTargetTempToRoomTemp(log, getState, setState, existsState, room, roomTemp, isInitialRun) {
	...
	if (
		!commonFunctions.isRoomIndoor(room) ||
		!hasAirConditioner(room) ||
		!isAirConditionerEnabled(log, getState, room) ||
		(mode != AIR_CONDITIONING_MODES.COOL && mode != AIR_CONDITIONING_MODES.HEAT)
	) {
		return;
	}

	const isCooling = mode == AIR_CONDITIONING_MODES.COOL;
	let targetTempLimit = getACTargetTempLimit(getState, existsState, room, isCooling);
	...
	let newTargetTemp = currentTarget;

	// Anpassung für Modus "Kühlen"
	if (isCooling) {
		if (!presenceFunctions.isAnybodyAtHome(getState)) {
			targetTempLimit += 1;
		}
		...
		if (!isInitialRun && currentTarget < targetTempLimit) {
			return;
		}

		// Soll-Temp schrittweise um 'targetTempAdjustmentStep'°C senken, bis untere Grenze erreicht
		newTargetTemp = roomTemp - ACTargetTempAdjustmentStep;
		...
		if (isInitialRun && newTargetTemp > currentTarget) {
			newTargetTemp = currentTarget;
		}
		...
		if (!isInitialRun && newTargetTemp == currentTarget) {
			newTargetTemp -= ACTargetTempAdjustmentStep;
		}

		// Bei minTargetTemp deckeln
		if (newTargetTemp < targetTempLimit) {
			newTargetTemp = targetTempLimit;
		}
	} else {
 ...
 * @returns {number} Limit for automatic temperature adjustment depending on AC operation mode
 */
...
/**
 * @param {string} room via Rooms
 * @returns {object} Current AC operation mode as enum (via AIR_CONDITIONING_MODES)
 */
function getAirConditionerOperationMode(log, getState, room) {
	...
	switch (modeValue) {
		case AIR_CONDITIONING_MODES.HEAT.value:
			return AIR_CONDITIONING_MODES.HEAT;
		case AIR_CONDITIONING_MODES.DRY.value:
			return AIR_CONDITIONING_MODES.DRY;
		case AIR_CONDITIONING_MODES.COOL.value:
			return AIR_CONDITIONING_MODES.COOL;
		case AIR_CONDITIONING_MODES.VENT.value:
			return AIR_CONDITIONING_MODES.VENT;
		case AIR_CONDITIONING_MODES.AUTO.value:
			return AIR_CONDITIONING_MODES.AUTO;
		default:
			return AIR_CONDITIONING_MODES.UNDEF;
	}
}
...
/**
 * @param {string} room via Rooms
 * @returns {object} Current AC fan speed as enum (via AIR_CONDITIONING_FAN_SPEEDS)
 */
function getAirConditionerFanSpeed(log, getState, room) {
	...
	switch (fanSpeedValue) {
		case AIR_CONDITIONING_FAN_SPEEDS.AUTO.value:
			return AIR_CONDITIONING_FAN_SPEEDS.AUTO;
		case AIR_CONDITIONING_FAN_SPEEDS.LOWEST.value:
			return AIR_CONDITIONING_FAN_SPEEDS.LOWEST;
		case AIR_CONDITIONING_FAN_SPEEDS.LOW.value:
			return AIR_CONDITIONING_FAN_SPEEDS.LOW;
		case AIR_CONDITIONING_FAN_SPEEDS.MEDIUM.value:
			return AIR_CONDITIONING_FAN_SPEEDS.MEDIUM;
		case AIR_CONDITIONING_FAN_SPEEDS.HIGH.value:
			return AIR_CONDITIONING_FAN_SPEEDS.HIGH;
		case AIR_CONDITIONING_FAN_SPEEDS.MAX.value:
			return AIR_CONDITIONING_FAN_SPEEDS.MAX;
		default:
			return AIR_CONDITIONING_FAN_SPEEDS.UNDEF;
	}
}
...
/**
 * @param {string} room via Rooms
 * @returns {object} Current direction of vertical vane as enum (via AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS)
 */
function getAirConditionerVaneVerticalDirection(log, getState, room) {
	...
	switch (vaneVerticalValue) {
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.AUTO.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.AUTO;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.TOPMOST.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.TOPMOST;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.UP.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.UP;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.MIDDLE.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.MIDDLE;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.DOWN.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.DOWN;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.BOTTOMMOST.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.BOTTOMMOST;
		case AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.SWING.value:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.SWING;
		default:
			return AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS.UNDEF;
	}
}
...
/**
 * @param {string} room via Rooms
 * @returns {object} Current direction of horizontal vane as enum (via AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS)
 */
function getAirConditionerVaneHorizontalDirection(log, getState, room) {
	...
	const vaneHorizontalValue = getState(
		idAliasAirConditionerPrefix + room.replace(" ", "") + idVaneHorizontalSuffix,
	).val;

	switch (vaneHorizontalValue) {
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.AUTO.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.AUTO;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.LEFTMOST.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.LEFTMOST;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.LEFT.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.LEFT;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.MIDDLE.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.MIDDLE;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.RIGHT.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.RIGHT;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.RIGHTMOST.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.RIGHTMOST;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.FIFTY_FIFTY.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.FIFTY_FIFTY;
		case AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.SWING.value:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.SWING;
		default:
			return AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS.UNDEF;
	}
}
...
module.exports = {
	AIR_CONDITIONING_MODES,
	AIR_CONDITIONING_FAN_SPEEDS,
	AIR_CONDITIONING_VANE_VERTICAL_DIRECTIONS,
	AIR_CONDITIONING_VANE_HORIZONTAL_DIRECTIONS,
	hasAirConditioner,
	adjustAllACTargetTempsToRoomTemp,
	adjustACTargetTempToRoomTemp,
	getACTargetTempLimit,
	setAirConditionerOperationMode,
	getAirConditionerOperationMode,
	enableAirConditioner,
	isAirConditionerEnabled,
	isAnyAirConditionerEnabled,
	getAirConditionerRoomTemperature,
	setAirConditionerTargetTemperature,
	getAirConditionerTargetTemperature,
	setAirConditionerFanSpeed,
	getAirConditionerFanSpeed,
	setAirConditionerVaneVerticalDirection,
	getAirConditionerVaneVerticalDirection,
	setAirConditionerVaneHorizontalDirection,
	getAirConditionerVaneHorizontalDirection,
};
