...
function adjustAllACTargetTempsToRoomTemp(log, getState, existsState, indoorTemps) {
    commonDefines.ROOMS_WITH_AIR_CONDITIONING.forEach(room => {
        setTimeout(() => {
            room = commonFunctions.translateRoomNameToEnglish(room);
            adjustACTargetTempToRoomTemp(log, getState, existsState, room, indoorTemps[room], true);
        }, 0);
    });
}
...
*/
function adjustACTargetTempToRoomTemp(log, getState, existsState, room, roomTemp, isInitialRun) {
    room = commonFunctions.translateRoomNameToEnglish(room);
