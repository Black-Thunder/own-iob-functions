...
/**
 * Sets the states of a pattern either to a fixed value
 * or modifies the current value by an offset.
 *
 * @param {string} pattern - e.g. "hm-rpc.0.*.LEVEL"
 * @param {any} [value] - fixed value if set
 * @param {number} [offset] - offset if specified (e.g. +10 or -5)
 */
async function setStatesByPatternAsync(
	$,
	getStateAsync,
	setStateAsync,
	pattern,
	...
	for (const id of states) {
		...
		if (offset !== undefined) {
			...
			if (current && typeof current.val === "number") {
				newVal = current.val + offset;
			} else {
				// Ignore non-numeric states as offset cannot be applied
				continue;
			}
		} else {
			newVal = value;
...
function getEnumFunctionsCachedObject(getObject, id) {
	return getCachedObject(getObject, id, "functions");
}

function getCachedObject(getObject, id, role) {
	const key = `${id}|${role}`;
	if (objectCache.has(key)) {
		return objectCache.get(key);
	}

	const obj = getObject(id, role);
 ...
 * @returns {(string|number|null)} Gibt den Wert von DefaultValue" zurück oder null wenn nicht zugeordnet
 */
function getDefaultValueEnum(existsState, getObject, stateId) {
	...
	const obj = getEnumFunctionsCachedObject(getObject, stateId);
	if (!obj || !obj.enumNames) {
		return null;
	}

	for (const name of obj.enumNames) {
		if (name.startsWith("DefaultValue")) {
			const value = name.slice("DefaultValue".length);
 ...
 */
async function wasIoBrokerRebooted(getStateAsync) {
	const os = require("os");
	const hostname = os.hostname();
	const uptimeState = await getStateAsync(`system.host.${hostname}.uptime`);

	if (!uptimeState) {
		return false;
	}

	const uptime = uptimeState.val;
	return uptime < 5 * 60;
}
...
/**
 * Determine whether a device is reachable by checking alias states.
 *
 * This function looks for two alias states under `alias.0.<id>` in the following order:
 * 1. `alias.0.<id>.WORKING` — if present, the device is considered reachable when its `val` is truthy.
 * 2. `alias.0.<id>.UNREACH` — if `WORKING` is absent but `UNREACH` is present, the device is considered
 *    reachable when `UNREACH.val` is falsy (i.e., reachable = !UNREACH.val).
 * If neither state exists, or the retrieved state is null/undefined, the function resolves to `false`.
 *
 * @param {string} id - Device identifier used to construct the alias state IDs (`alias.0.<id>.WORKING` and `alias.0.<id>.UNREACH`).
 * @returns {Promise<boolean>} Promise that resolves to `true` if the device is considered reachable, otherwise `false`.
 */
async function isDeviceReachable(getStateAsync, existsState, id) {
	const workingStateId = `alias.0.${id}.WORKING`;
	if (existsState(workingStateId)) {
		const state = await getStateAsync(workingStateId);
		return state ? state.val : false;
	}

	const unreachStateId = `alias.0.${id}.UNREACH`;
	if (existsState(unreachStateId)) {
		const state = await getStateAsync(unreachStateId);
		return state ? !state.val : false;
	}
