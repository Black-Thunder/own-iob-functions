...
* @param {boolean} returnEnglishName Statt deutschem Namen den englischen zurückgeben (unter "enum.rooms" muss bei allen Räumen "de" und "en" eingetragen sein)
    *   "common": {
            "name": {
                "en": "RoomDomi",
                "de": "Zimmer Domi"
...
*/
function getAssignedRoomFromState(getObject, stateId, returnEnglishName = false, roomIndex = 0) {
    const rooms = getObject(stateId, "rooms");
    const roomNames = rooms ? rooms.enumNames : "";

    // bei vordefinierten Räumen kommt ein JSON mit Übersetzungen mit, ansonsten nur der selbstvergebene Name
    return roomNames == "" ? "" : (returnEnglishName ? roomNames[roomIndex].en : roomNames[roomIndex].de);
}
...
module.exports = {
    roundValue, clamp, occurrences, isRoomIndoor, getAssignedRoomFromState, getDefaultValueEnum, stringOfEnum, mapStateValueToStateEnum, translateRoomNameToEnglish, translateRoomNameToGerman, mapRange, wasIoBrokerRebooted, getStateText
};