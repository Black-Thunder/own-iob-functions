...
function setThermostatMode(log, existsState, getState, setState, compareTime, room, mode) {
    ...
    if (!isSummerActive) {
        switch (mode) {
            // Hier kann der Modus direkt auf "AUTO" gesetzt werden. Soll-Temperatur muss danach erneut bestimmt und gesetzt werden (kurz verzögert)
            case THERMOSTAT_MODES.AUTO:
                ...
                break;
            // Hier muss der Modus auf "MANUAL" und die Solltemperatur auf 5°C gesetzt werden
...
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room) || (getThermostatMode(getState, room) == THERMOSTAT_MODES.MANUAL && getThermostatTargetTemp(getState, room) == minThermostatTargetTemp) || isThermostatSummerActive(getState, room)) return; // Heizung aus (== MANUAL und 5° Soll) oder Sommermodus aktiv
