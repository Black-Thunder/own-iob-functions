...
function setThermostatMode(log, existsState, getState, setState, compareTime, room, mode) {
    ...
    if (!isSummerActive) {
        switch (mode) {
            // Hier kann der Modus direkt gesetzt werden. Soll-Temperatur muss danach erneut bestimmt und gesetzt werden (kurz verzögert)
            case THERMOSTAT_MODES.AUTO:
            case THERMOSTAT_MODES.MANUAL:
                setThermostatModeDirectly(setState, room, mode);
                // Anpassung der Solltemperatur in diesem Fall erzwingen (-> true), da sich Thermostat noch im "CLOSED"-Modus befindet und sonst keine Anpassung erfolgen würde
                setTimeout(() => { adjustThermostatTargetTemp(log, getState, setState, compareTime, room, sensorFunctions.getCurrentInsideTemperature(existsState, getState, room), true); }, 500);
                break;
            // Simulation des CLOSED Modus: Hier muss der Modus auf "MANUAL" und die Solltemperatur auf 5°C gesetzt werden
            case THERMOSTAT_MODES.CLOSED:
                ...
                setThermostatValveLevel(getState, setState, room, 0);
                break;
        }
    }
}
...
function setThermostatValveLevel(getState, setState, room, level) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room) || isThermostatSummerActive(getState, room)) return; // wenn Sommerschaltung aktiv, nichts setzen, sonst schaltet sich das Thermostat ein
    level = commonFunctions.clamp(level, 0, 100);
...
* @param {boolean} [ignoreClosedMode=false] Anpassung der Temperatur erzwingen und "CLOSED" ignorieren
*/
function adjustAllThermostatsTargetTemps(log, getState, setState, getObject, compareTime, indoorTemps, ignoreClosedMode = false) {
    var radiators = getObject("enum.functions.heating").common.members;
    ...
    for (let i = 0; i < radiators.length; i++) {
        setTimeout(() => {
            ...
            room = commonFunctions.translateRoomNameToEnglish(room);
            adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemps[room], ignoreClosedMode);
        }, 0);
    }
}
...
* @param {boolean} [ignoreClosedMode=false] Anpassung der Temperatur erzwingen und "CLOSED" ignorieren
*/
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp, ignoreClosedMode = false) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    const isClosedMode = ignoreClosedMode ? false : (getThermostatMode(getState, room) == THERMOSTAT_MODES.MANUAL && getThermostatTargetTemp(getState, room) == minThermostatTargetTemp);
    if (!hasThermostat(room) || isClosedMode || isThermostatSummerActive(getState, room)) return; // Heizung aus (== MANUAL und 5° Soll) oder Sommermodus aktiv
...
module.exports = {
    ...
    getThermostatTargetTemp, getThermostatDayTemp, getThermostatNightTemp, getCurrentThermostatTemp, getThermostatMode,
    getThermostatBatteryVoltage, getThermostatValveLevel, setThermostatTargetTemperature, setThermostatModeDirectly, setAllThermostatsModes, setThermostatValveLevel,
    adjustAllThermostatsTargetTemps, adjustThermostatTargetTemp, isThermostatSummerActive, isThermostatDaytimeSchedule, getThermostatSchedulingTimes
};