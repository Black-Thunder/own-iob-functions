...
var weatherSensorFunctions = require("/opt/iobroker/iobroker-data/modules/weatherSensorFunctions.js");
...
const idIsSummerModeActiveSuffix = ".IS_SUMMER_MODE_ACTIVE";
...
*/
function setThermostatMode(getState, setState, room, mode) {
    if (mode != THERMOSTAT_MODES.CLOSED && mode != THERMOSTAT_MODES.OPEN && mode != THERMOSTAT_MODES.AUTO) {
        return;
    }

    room = commonFunctions.translateRoomNameToEnglish(room);
    const oldMode = getThermostatMode(getState, room);
    const isSummerActive = isThermostatSummerActive(getState, room);

    // Nur wenn keine Sommerschaltung vorliegt und sich der neue Modus vom alten unterscheidet
    if (!isSummerActive && oldMode != mode) {
...
* @param {number} temp
*/
function setThermostatTargetTemperature(setState, room, temp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
...
function adjustAllThermostatsTargetTemps(log, getState, getObject, compareTime, indoorTemps) {
    var radiators = getObject("enum.functions.heating").common.members;
    ...
    for (let i = 0; i < radiators.length; i++) {
        setTimeout(() => {
            ...
            room = commonFunctions.translateRoomNameToEnglish(room);
            adjustThermostatTargetTemp(log, getState, compareTime, room, indoorTemps[room]);
        }, 0);
    }
}
...
* @param {string} room via Rooms
*/
function adjustThermostatTargetTemp(log, getState, compareTime, room, indoorTemp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    ...
    if (adjustedTargetTemp != currentTargetTemp) {
        setThermostatTargetTemperature(setState, room, adjustedTargetTemp);
    }
}
/// ENDE SET
...
function isThermostatSummerActive(getState, room) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room)) return false;
    return getState(`${idAliasThermostatPrefix}${room}${idIsSummerModeActiveSuffix}`).val
}
