...
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    const isThermostatClosedMode = (getThermostatWindowState(getState, room) == THERMOSTAT_WINDOW_STATES.OPEN);
    if (!hasThermostat(room) || isThermostatClosedMode || isThermostatSummerActive(getState, room)) return; // Heizung aus oder Sommermodus aktiv
