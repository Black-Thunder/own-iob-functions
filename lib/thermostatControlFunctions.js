...
function getAdjustedTargetTemp(log, getState, compareTime, room, indoorTemp) {
    ...
    const maxAdjustmentLimit = getState(`${commonDefines.idUserDataPrefix}ThermostatControl.AutomaticTempAdjustmentLimit`).val;
    const maxAdjustedScheduledTemp = scheduledTemp + maxAdjustmentLimit;

    // Evtl. noch an Innentemperatur angleichen
    var adjustedScheduledTemp = adjustTargetTempToInsideTemp(getState, scheduledTemp, room, indoorTemp);

    // Änderung der Soll-Temperatur begrenzen
    if (adjustedScheduledTemp > maxAdjustedScheduledTemp) adjustedScheduledTemp = maxAdjustedScheduledTemp;
...
function getThermostatMode(getState, room) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room)) return -1;
    var mode = getState(`${idAliasThermostatPrefix}${room}${idOperationModeSuffix}`).val;
...
function setThermostatTargetTemperature(getState, setState, room, temp) {
    ...
    if (isThermostatSummerModeActive(getState, room) || getThermostatWindowState(getState, room) == THERMOSTAT_WINDOW_STATES.OPEN) {
...
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room) || isThermostatSummerModeActive(getState, room) || (getThermostatWindowState(getState, room) == THERMOSTAT_WINDOW_STATES.OPEN)) return; // wenn Sommermodus aktiv oder Fenster geöffnet, nichts tun
...
module.exports = {
    THERMOSTAT_MODES, THERMOSTAT_WINDOW_STATES, hasThermostat, setThermostatMode, getAdjustedTargetTemp, getScheduledTargetTemp, adjustTargetTempToOutsideTemp, adjustTargetTempToInsideTemp,
