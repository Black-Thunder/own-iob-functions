...
function setAllThermostatsModes(getState, setState, getObject, mode) {
	const radiators = getObject("enum.functions.heating").common.members;
 ...
 */
function adjustAllThermostatsTargetTemps(log, getEnums, getState, setState, getObject, compareTime, indoorTemps) {
	const radiators = getObject("enum.functions.heating").common.members;

	for (let i = 0; i < radiators.length; i++) {
		setTimeout(() => {
			let room = commonFunctions.getRoomFromState(getEnums, radiators[i]);
			room = commonFunctions.translateRoomNameToEnglish(room);
