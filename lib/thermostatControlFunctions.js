...
function setThermostatTargetTemperature(getState, setState, room, temp) {
    ...
    if (isThermostatSummerActive(getState, room)) temp = minThermostatTargetTemp; // wenn Sommerschaltung aktiv, immer Minimalsolltemperatur erzwingen, damit Heizung aus bleibt
...
function setThermostatValveLevel(getState, setState, room, level) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room)) return;
    if (isThermostatSummerActive(getState, room)) level = minThermostatValveLevel; // wenn Sommerschaltung aktiv, immer Ventil ganz schließen erzwingen, damit Heizung aus bleibt
...
function setThermostatWindowState(getState, setState, room, state) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room)) return;
    if (isThermostatSummerActive(getState, room)) state = THERMOSTAT_WINDOW_STATES.OPEN; // wenn Sommerschaltung aktiv, immer "WINDOW_OPEN" erzwingen, damit Heizung aus bleibt
...
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp) {
    room = commonFunctions.translateRoomNameToEnglish(room);
    if (!hasThermostat(room) || isThermostatSummerActive(getState, room) || (getThermostatWindowState(getState, room) == THERMOSTAT_WINDOW_STATES.OPEN)) return; // Heizung aus oder Sommermodus aktiv
...
module.exports = {
    ...
    setThermostatTargetTemperature, setThermostatModeDirectly, setAllThermostatsModes, setThermostatValveLevel, setThermostatWindowState,
