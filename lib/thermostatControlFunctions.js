...
// -------------------------------------------------
// ROBUST TARGET TEMP HANDLING
// -------------------------------------------------
const pendingTargetChecks = {}; // room -> { target, retries, failed, generation }
const roomWriteLocks = {}; // Debounce je Raum

const MAX_RETRIES = 3;
const VERIFY_DELAY = 15_000; // ms
...
function setThermostatMode(log, existsState, getState, setState, compareTime, room, mode) {
	...
	switch (mode) {
		...
		case THERMOSTAT_MODES.CLOSED:
			...
			setThermostatTargetTemperature(log, getState, setState, room, minThermostatTargetTemp);
			break;
	}
}
 ...
 * @param {number} temp
 */
function setThermostatTargetTemperature(log, getState, setState, room, temp) {
	room = commonFunctions.translateRoomNameToEnglish(room);
	if (!hasThermostat(room)) {
		return;
	}
	...
	// Debounce pro Raum
	clearTimeout(roomWriteLocks[room]);
	roomWriteLocks[room] = setTimeout(() => {
		internalSetThermostatTargetTemperature(log, getState, setState, room, temp);
	}, 800);
}

function internalSetThermostatTargetTemperature(log, getState, setState, room, temp) {
	const currentTargetTemp = getThermostatTargetTemp(getState, room);
	if (currentTargetTemp == temp) {
		return;
	}

	pendingTargetChecks[room] = {
		target: temp,
		retries: 0,
		failed: false,
		generation: Date.now(), // invalidiert alte Timer
	};

	writeThermostatTargetTemp(setState, room, temp);
	scheduleTargetTempVerify(log, getState, setState, room);
}

function scheduleTargetTempVerify(log, getState, setState, room) {
	const check = pendingTargetChecks[room];
	if (!check) {
		return;
	}
	...
	setTimeout(() => {
		const check = pendingTargetChecks[room];
		if (!check || check.generation !== generation) {
			return; // veralteter Timer → ignorieren
		}

		verifyThermostatTargetTemp(log, getState, setState, room);
	}, VERIFY_DELAY);
}

function verifyThermostatTargetTemp(log, getState, setState, room) {
	const check = pendingTargetChecks[room];
	if (!check) {
		return;
	}

	const actual = getThermostatTargetTemp(getState, room);

	if (Math.abs(actual - check.target) <= TEMP_TOLERANCE) {
		delete pendingTargetChecks[room];
		return;
	}
	...
	if (check.retries >= MAX_RETRIES) {
		check.failed = true;
		log(
			`🔥 Thermostat ${room}: Zieltemperatur ${check.target}°C konnte nicht gesetzt werden! Aktuell: ${actual}°C`,
			"error",
		);
		return;
	}
	...
	setTimeout(() => {
		const current = pendingTargetChecks[room];
		if (!current || current.generation !== generation) {
			return;
		}

		writeThermostatTargetTemp(setState, room, check.target);
		scheduleTargetTempVerify(log, getState, setState, room);
	}, RETRY_DELAY);
}

function writeThermostatTargetTemp(setState, room, temp) {
	setState(`${idAliasThermostatPrefix}${room}${idTargetTemperature}`, temp);
}

function getThermostatTargetTempFailures() {
	return Object.entries(pendingTargetChecks)
		.filter(([_, v]) => v.failed)
		.map(([room]) => room);
}
...
function adjustThermostatTargetTemp(log, getState, setState, compareTime, room, indoorTemp) {
	...
	setThermostatTargetTemperature(log, getState, setState, room, adjustedTargetTemp);
}
/// ENDE SET
...
module.exports = {
	...
	getThermostatTargetTempFailures,
