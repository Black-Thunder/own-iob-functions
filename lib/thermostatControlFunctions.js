...
function adjustTargetTempToInsideTemp(getState, compareTime, targetTemp, room, indoorTemp) {
    ...
    // Wenn reelle Temperatur bereits Soll-Temp erreicht hat, Ziel-Temp unterhalb der Ventil-Temp anpassen
    if (indoorTemp >= targetTemp) {
        // tagsüber auf 1.5° unter Ventiltemperatur setzen, nachts 2°
        const tempReduction = isThermostatDaytimeSchedule(getState, compareTime, room) ? 1.5 : 2;
        return Math.round((thermostatTemp - tempReduction) * 2) / 2;
    }
